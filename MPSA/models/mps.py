...
class MultiPartsSampling(nn.Module):
	...
	def feature_weights_pooling(self, x, feature_weights):
		...
		# 数值稳定性修复：防止除零导致梯度爆炸
		denominator = sum_feature_weights.sum(-2).unsqueeze(-1)
		denominator = torch.clamp(denominator, min=1e-6)  # 防止除零和数值不稳定
		sum_feature_weights = sum_feature_weights / denominator
	...
	def normalize_cam(self,grad):
		...
		grad = grad - min
		# 数值稳定性修复：增强混合精度训练下的稳定性
		denominator = grad.sum(-1, keepdim=True)
		denominator = torch.clamp(denominator, min=1e-5)  # 比1e-8更安全，适应混合精度
...
class PartSamplingAttention(nn.Module):
	...
	def forward(self, x, parts):
		...
		# 数值稳定性修复：防止特征权重归一化时的除零错误
		min_val = feature_weights.min()
		max_val = feature_weights.max()
		denominator = max_val - min_val
		denominator = torch.clamp(denominator, min=1e-6)  # 防止除零
		feature_weights = (feature_weights - min_val) / denominator
